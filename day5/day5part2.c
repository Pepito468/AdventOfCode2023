...
/*PART 2 UPDATE: I basically change the way filling seeds work. the rest remains the same*/
int main(int argc, char* argv[]){
    FILE *p;
    int height =0;
    //file
    char buffer[10000000][10000000];
    int i,j,k,h;
    //slider
    int slider;

    //seed container but will transform into fetilizer and so on...
    long int res_container[100000000];
    long int res_num=0;
    long int res_slid;
    //step counter to know at step I am at
    int stepcounter = -1;
    //filled every line  -> is the array I will use to evolve my seeds
    long int checker[100000000];
    int z=0;
    ...
    printf("\n");
        ...
        switch (stepcounter) {
            case 0:
                ...
                for (k=0;k<checker_num;k+=2){
                    h=0;
                    while (checker[k+1] > 0){
                        res_container[z] = checker[k]+h;
                        h++;
                        z++;
                        checker[k+1]--;
                    }
